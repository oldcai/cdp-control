...
- **固定端口门禁(`browser-port.ts`)**:对 `CDP_HOST` 的全部数值地址逐一 connect + exclusive bind 区分空闲/忙/未知；`localhost` 覆盖 IPv4/IPv6 双回环，DNS 用 `all:true`，bracketed IPv6 在 socket 边界去括号，listener 匹配会规范化 IPv6 等价写法。只有原始 `CDP_HOST` 本身是数值地址时才保留一次 GET 快路；所有 DNS hostname 即使只解析到一个地址也必须逐数值地址验证后再 pin，多地址则无论主连接是否健康都复核并 pin 到具体健康端点。无法归属时 fail closed，绝不回收多地址 listener 并集或让后续连接漂移。一次 ensure/kill 尝试内探活、端口状态、listener 快照及 spawn 后就绪等待共用同一解析快照；每个耗时步骤后和逐 listener kill 前复核，地址集合变化则 fail closed。忙端口先给并发冷启动 3s 就绪宽限，再用精确 TCP LISTEN 快照归属全部 PID，并同步绑定 Linux start ticks、macOS start marker 或 Windows creation ticks；signal 前无 await 地复核同一 birth identity，PID 被复用时也会 fail closed。破坏性操作前会复探健康状态并核对 listener 身份，回收会尝试整组 listener、聚合失败并确认端口释放；任何枚举、身份、状态、回收或释放异常都 fail closed。此模块只返回复用或允许在原端口启动，不提供端口避让能力。
