...
- **固定端口门禁(`browser-port.ts`)**:网络与进程副作用由 `browser.ts` 注入，核心编排纯测。connect+严格 bind 区分 free/busy/unknown；netstat/lsof 解析只认服务目标 host/精确端口的 TCP LISTEN，并按地址族归属、PID 去重。忙端口先给并发冷启动 3s 宽限，随后在破坏性操作前复探健康状态并复核 PID 快照；回收后再次确认端口释放并在 spawn 前重走门禁。子进程早退也会在同一配置端口有界复探并发启动者，仍未健康才保留真实 exit code/signal 报错。`src/port.ts` 的动态挑口工具不在浏览器启动路径中使用。
