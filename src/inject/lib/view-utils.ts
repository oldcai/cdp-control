...
export function inlineLen(n: TextTreeNode): number {
...
export function inlineable(n: TextTreeNode): boolean {
  const l = inlineLen(n);
  return l > 0 && l <= 24;
}
...
export function leafText(n: TextTreeNode): string {
...
export function firstTxt(arr: TextTreeNode[]): string {
  for (const k of arr) {
...
export function isTrivialLeaf(n: TextTreeNode): boolean {
