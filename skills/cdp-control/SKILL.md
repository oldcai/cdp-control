...
环境变量:`CDP_HOST`(默认 `127.0.0.1`)、`CDP_LOGS_PORT`(daemon,默认 9333)、`CDP_USER_DATA`(默认 `~/.cdp-control/user-data`)、`CDP_RULES_DIR`(实时规则目录,默认 `~/.cdp-control/rules`)、`CDP_FOLD_FILE`/`CDP_IGNORE_LINKS_FILE`(单文件路径覆盖,测试用)。浏览器端口只认 `browser.json.port`；无配置 bootstrap 固定 9222。`CDP_PORT` 不覆盖这条固定端口契约。
...
- 连接失败 → 先 `list` 自动启动；仍失败检查 `CDP_HOST` 与 `browser.json.port`（无配置固定 9222）。
