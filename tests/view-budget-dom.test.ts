...
interface RefEntry { el: Element; parentRef: number | null }
...
test('构造 DOM: buildView 先分配全部 ref，预算折叠只替换渲染且保留可展开句柄', () => {
  ...
  assert.equal(refsBefore[2].el, paragraph as unknown as Element);
  assert.equal(refsBefore[3].el, button as unknown as Element);
});
